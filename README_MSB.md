...
def ovh_decode(data, key):
    if len(data) <= 127:
        return sn_decode_ovh(data, key)
    else:
        # 前128字节特殊处理
        result = sn_decode_ovh(data[:128], key)
        # 后续字节XOR处理
        result += data_dec_last_part(data[128:], len(data) - 128)
        return result
```
...
- 逆向分析文章: https://www.jarvisw.com/?p=1132
- MuseScore开源项目: https://github.com/musescore/MuseScore
